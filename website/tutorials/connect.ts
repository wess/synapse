  ...
  body: `
    ...
    <ol class="steps">
      <li>
        ...
        <p>Extract <code>synapse.zip</code>, move <strong>synapse.app</strong> to Applications, then open it. macOS should accept the notarized Developer ID application without a security workaround.</p>
...
synapse path`)}
      ...
      <li>
        ...
        <p>Synapse registers <code>~/.local/bin/synapse mcp</code> at user scope and adds a managed memory instruction block. Existing configuration and instruction text remain in place; changed existing files receive <code>.synapsebackup</code> siblings.</p>
      </li>
      <li>
        <h3>Restart the developer tool</h3>
        <p>Close and reopen the connected tool so it loads the new MCP server. Inspect its available tools or ask it what Synapse tools are available. The answer should include <code>remember</code>, <code>recall</code>, and <code>vaultstatus</code>.</p>
      </li>
      <li>
