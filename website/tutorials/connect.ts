...
cli	/Users/example/.local/bin/synapse`)}
      ...
      <li>
        ...
        <p>Return to the dashboard. Each supported tool appears as a row with its state. Choose <strong>Connect</strong> beside Codex or Claude Code, and the row moves from detected to connected. Connect the second tool too if you have it — the whole point is one memory behind both.</p>
...
pointers	2/2
consolidated	false`)}
        <p><code>pointers 2/2</code> means both tools point at one editable <code>SOUL.md</code>. That file is yours to edit — it is where shared guidance for every connected tool lives.</p>
      </li>
