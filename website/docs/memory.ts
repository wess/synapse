...
synapse memory show 24`)}
    ...
    <p>It is laid out in three dimensions: drag to turn it, scroll to move closer, click a point to read it. It is the first screen in the app and the first page of the terminal dashboard, which draws the front of the same cloud in braille. Both read the newest 160 memories and say so when the store is larger, and neither changes anything.</p>
...
synapse settings optimize balanced`)}
    ...
    <p>Similar wording is not enough to discard a memory: negation, numbers, and short identifiers can change the meaning. Distinct bodies remain eligible until explicitly superseded. Search keeps non-Latin terms instead of treating them as empty queries.</p>
