...
export const security: Page = {
  ...
  body: `
    ...
    <p>Synaps is local-first, not a sandbox. It removes unnecessary network and file exposure from the memory-and-credential workflow, but it still runs as your macOS user. Any process already able to control your account, inspect your terminal, or access an unlocked Keychain may sit inside the same trust boundary.</p>
    <p>The application requires no Synaps account or hosted service. MCP uses local stdio. Memory and metadata live in a local SQLite file. The beta does not include telemetry, remote synchronization, or a web server.</p>
    ...
    <p>An unapproved, edited, or invalid <code>.synaps.yaml</code> never contributes variables. Approval stores a SHA-256 digest of the exact file bytes and its canonical path. Any later edit invalidates that digest.</p>
    <p>Resolution walks from broad ancestor folders toward the working folder. Narrower approved files can replace a mapping, but a name denied by a broader scope remains denied. Any scope warning prevents <code>synaps run</code> from launching and makes the shell hook unload its managed values, avoiding silent partial configuration.</p>

    <h2 id="children">Environment boundaries</h2>
    <p><code>synaps run -- &lt;command&gt;</code> reads only the selected Keychain values and places them in the environment of a new child process. That command and all of its descendants can read those values normally. Synaps cannot stop the child from logging, transmitting, or persisting them.</p>
    ...
    <p>Settings installs the hook inside a marked startup-file block using the absolute managed CLI path. It backs up and atomically replaces an existing file, refuses incomplete or duplicate Synaps blocks, and removes only the marked block when disabled.</p>
    ...
    <ul>
      ...
      <li>Use <code>synaps status .</code> immediately before a sensitive command.</li>
      <li>Use <code>synaps deny</code> when a directory should no longer activate automatically.</li>
    </ul>

    <h2 id="limits">Operational limits</h2>
    <ul>
      <li>Synaps does not isolate a malicious connected MCP client from recalled memory. Only connect tools you trust.</li>
      ...
      <li>There is no remote revocation system. Forget a secret in Synaps and rotate it at its issuer when exposure is possible.</li>
    </ul>
  `,
};
