...
export const install: Page = {
  ...
  body: `
    ...
    <ul>
      ...
      <li>A writable <code>~/.local/bin</code>, or a custom path supplied through <code>SYNAPS_BIN</code>.</li>
    </ul>
    ...
    <p>Open <strong>Settings</strong> in Synaps and choose <strong>Install CLI</strong>, or run the application binary once from a terminal and use:</p>
    ${code("shell", `synaps install
synaps path
synaps version`)}
    <p>The default destination is <code>~/.local/bin/synaps</code>. If your shell cannot find it, add that folder to <code>PATH</code> and start a new shell.</p>
    ${code("shell", `export PATH="$HOME/.local/bin:$PATH"`)}
    <p>Synaps never overwrites an unrelated executable. A managed launcher has a sibling <code>.synapsreceipt</code> containing its digest so future installs can distinguish an update from a conflict.</p>

    <h2 id="shell">Enable shell integration</h2>
    <p>In <strong>Settings → Shell environments</strong>, choose <strong>Enable shell hook</strong>. Synaps detects your default zsh, bash, or fish shell, installs the CLI if needed, and adds one marked block to its startup file. Open a new terminal after the setting changes.</p>
    <table>
    ...
    <p>The app shows <strong>Needs repair</strong> if the managed block changes. Repair replaces only that block. Remove deletes only that block; already-running terminals retain the loaded hook until they close. Every changed existing startup file receives a <code>.synapsbackup</code> sibling and an atomic replacement.</p>

    <h2 id="tools">Connect tools</h2>
    <p>On the Synaps dashboard, each detected tool shows its installation and connection state. Choose <strong>Connect</strong> for Codex or Claude Code.</p>
    <p>Setup performs two changes as one rollback-protected operation:</p>
    <ol>
      <li>It registers the installed Synaps executable as a user-level MCP stdio server with the single argument <code>mcp</code>.</li>
      <li>It appends or refreshes a delimited Synaps memory block in the tool’s global instruction file. Existing user content remains outside that block.</li>
    </ol>
    <table>
    ...
    <p>Before changing a tool store or instruction file, Synaps creates a sibling <code>.synapsbackup</code>. If either half of setup fails, both files are restored.</p>
    ...
    ${code("shell", `codex mcp add synaps -- ~/.local/bin/synaps mcp
claude mcp add --scope user synaps -- ~/.local/bin/synaps mcp`)}
    <p>Then add the memory instructions shown in the app to the corresponding global instruction file. Do not point an integration at a build artifact such as <code>target/debug/synaps</code>; cleaning the repository would break the connection.</p>

    <h2 id="verify">Verify the connection</h2>
    <ol>
      <li>Restart the connected tool so it reloads its MCP servers and instructions.</li>
      <li>Inspect its available tools. Synaps should expose <code>remember</code>, <code>recall</code>, and <code>vaultstatus</code>.</li>
      <li>Ask it to remember one harmless confirmed convention, then recall it in a new session.</li>
      <li>Open the Synaps <strong>Memories</strong> screen and confirm the exact entry and source are visible.</li>
    </ol>
