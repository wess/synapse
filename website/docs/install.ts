...
synapse version`)}
    ...
    <p>The connected tool follows that pointer on launch, and the MCP server loads the same <code>SOUL.md</code> during initialization. The default guidance tells every connected tool to recall global plus current-project context, save confirmed reusable facts with project scope by default, use a Lean response first, and keep secrets out of memory.</p>
    ...
    <p>Open <strong>Settings → Shared guidance</strong>. <strong>Open shared guidance</strong> edits the central <code>SOUL.md</code>. <strong>Sync pointers</strong> creates it if needed and refreshes every global pointer while preserving every line outside Synapse's managed blocks.</p>
    <p>If you want those global files to contain only those pointers, choose <strong>Consolidate guidance</strong> and confirm. Synapse moves their unmanaged content into <code>SOUL.md</code>, deduplicates simple shared rules, writes pointer-only global files, and keeps <code>.synapsebackup</code> siblings. This operation is never performed implicitly during connection setup.</p>
