...
synapse settings show`)}
    ...
    <p>Confirm you downloaded the current archive from the release page, extracted it fully, and moved the app to Applications. This prerelease is Developer ID signed but still awaits Apple notarization, so default Gatekeeper settings may require the <a href="../install/">documented Open Anyway step</a>. Avoid modifying files inside the application bundle because that invalidates its signature.</p>
