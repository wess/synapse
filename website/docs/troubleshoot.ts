...
synapse settings show`)}
    ...
    <p>Run <code>synapse guidance show</code>. If fewer than two pointers are ready, use <code>synapse guidance sync</code> or <strong>Settings → Shared guidance → Sync pointers</strong>, then restart both tools. Consolidation is optional and requires separate confirmation.</p>
    <h3>Setup failed</h3>
