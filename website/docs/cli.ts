...
export const cli: Page = {
  ...
  body: `
    ...
    ${command("run", "synapse run -- <command> [arguments]", "Resolve global, project, and folder vault mappings for the current directory, refuse on any scope warning, read the selected values out of the vault, and launch the child. Synapse returns the child exit code, or 1 when the operating system reports no code.")}
    ...
    ${command("status", "synapse status [folder] [--json]", "Show the resolved folder, available environment names, discovered scope states, warnings, ambient readiness, and detected shell hook. Reports which value store this machine uses and never a value. The folder defaults to the current directory.")}
    ...
    ${command("vault backend", "synapse vault backend [&lt;keychain|encrypted&gt;]", "Print which store holds this machine's secret values, or choose one. Choosing moves nothing, so it refuses once secrets exist and points at <code>vault migrate</code>. The encrypted store is the default on a new installation and the only one off macOS.")}
    ${command("vault migrate", "synapse vault migrate &lt;keychain|encrypted&gt; [--keep]", "Move every value into the other store: copy them all, read each one back, switch the setting, then remove the originals. <code>--keep</code> leaves the originals in place. A source that cannot be read stops the migration with nothing moved and nothing switched.")}
    ...
    ${command("secret set", "synapse secret set <vault> <name> <env> [--global]", "Read a value from a hidden terminal prompt or stdin and save it in the vault. Creates metadata when the label is new; replaces the stored value when the environment name matches. <code>--global</code> also enables the mapping.")}
    ${command("secret copy", "synapse secret copy <vault.name>", "Put one value on the clipboard. The only way a stored value comes back out, and it never reaches the terminal: the command prints the reference and nothing else. Set <code>SYNAPSE_CLIPBOARD</code> to use something other than <code>pbcopy</code>.")}
    ${command("secret forget", "synapse secret forget <vault.name>", "Delete the stored value, its metadata, and any global mapping. The value is removed from both stores, so a machine that has switched backends leaves nothing behind in the one it stopped reading.")}
    ...
    ${command("memory list", "synapse memory list [query] [--json] [--explain]", "Search the body with the joined query words or list recent memory when empty. Returns up to 100 entries. Text mode prints ID, scope, source, and a compact preview, and marks any memory that has been superseded. With <code>--explain</code>, print what the search actually did first: the full-text expression the query became, the words it searched for, the words it dropped for matching nearly everything, and the score behind each hit.")}
    ${command("memory grep", "synapse memory grep &lt;text&gt; [--json]", "Find memories containing an exact string, most recent first. Nothing is tokenized, ranked, or dropped \u2014 the characters given have to appear, in that order \u2014 which is what makes it the right tool for an identifier, a flag, a path, or a word the ranked search treats as noise. Matching is case-insensitive for ASCII, and a pattern after a bare <code>--</code> is taken literally, so <code>memory grep -- --no-verify</code> searches for the flag.")}
    ...
    ${command("memory supersede", "synapse memory supersede &lt;old&gt; &lt;new&gt;", "Stop recalling one memory in favour of another. The old one is not deleted: it keeps its id, stays listed and readable, says what replaced it, and comes back with <code>memory restore</code>. Use this instead of leaving a correction to argue with what it corrects.")}
    ${command("memory restore", "synapse memory restore &lt;id&gt;", "Put a superseded memory back in circulation. Deleting the memory that replaced it does the same thing on its own.")}
...
${command("memory undo", "synapse memory undo <batch> --confirm", "Remove memories created only by that import batch. Preserve manually edited memories and records linked to another origin.")}
    ...
    ${command("memory wipe", "synapse memory wipe --confirm", "Delete every memory entry and import batch while leaving SOUL.md, settings, vaults, stored values, and scope approvals intact. The exact guard is required.")}
    ...
    <p>Every command that takes a bare name resolves this project's shelf before the global one, so a repository's own <code>release</code> shadows the shared one rather than colliding with it. Pass <code>--global</code> or <code>--project [folder]</code> to say which you mean.</p>
    ${command("skill proposed", "synapse skill proposed [--json]", "List the skills agents have written that nobody has looked at yet, oldest first, with the tool that wrote each one and the line it left saying why. A proposed skill is in the library and in no tool.")}
    ${command("skill approve", "synapse skill approve &lt;name&gt; [--tool &lt;tool&gt;]", "Install a proposed skill where it belongs and stop calling it proposed. A global skill goes into every connected tool's personal skills folder; a project skill goes into that project's.")}
    ${command("skill reject", "synapse skill reject &lt;name&gt; --confirm", "Turn a proposed skill down. It leaves the library with its history, and nothing else on the machine ever knew about it. Only a skill still waiting for review can be rejected \u2014 once one is approved it is an ordinary skill, and <code>skill delete</code> is what removes it.")}
    ${command("skill history", "synapse skill history &lt;name&gt; [--json]", "Show what a skill used to say, newest first, with the line saying what was wrong with each version. The newest twenty are kept.")}
    ${command("skill revert", "synapse skill revert &lt;name&gt; [&lt;id&gt;]", "Put an earlier version back, in the library and in every tool holding a copy Synapse wrote. What it said is kept as a new revision, so a revert can itself be reverted.")}
    ...
    ${command("data export", "synapse data export <file>", "Create and validate a consistent SQLite snapshot of <code>brain.db</code> at a destination that does not already exist. Secret values live in a separate store — Keychain, or <code>vault.db</code> beside it — and are never in the snapshot.")}
    ...
    ${command("settings workers", "synapse settings workers &lt;count&gt;", "The most background workers one session may run at once. Every worker is a separate session on the account you already pay for, so this is a spending limit as much as a performance one. Clamped to a built-in ceiling, so a mistyped number cannot buy an unbounded fleet. A supervisor already running picks the change up on its next spawn.")}
    ${command("settings reactor", "synapse settings reactor &lt;on|off&gt;", "Draw the reactor in the middle of the app's console, or leave the column to the numbers beside it. Nothing is lost by turning it off. A build made without the <code>reactor</code> feature has none to draw whatever this says.")}
    ${command("settings learn", "synapse settings learn &lt;on|off&gt;", "Let agents write and correct skills, adding <code>teach</code> and <code>revise</code> to every connected session. Off by default. A skill an agent writes waits in <code>skill proposed</code> until you approve it, so turning this on never changes how a session behaves without you having read what changed it.")}
...
${command("guidance adopt", "synapse guidance adopt --confirm", "Move unmanaged global guidance into SOUL.md, replace both global files with managed pointers, and retain backups.")}
    ...
    ${command("connect", "synapse connect [tool]", "Wire a tool into memory and the vault: register the Synapse MCP server through that tool's own CLI, and point its global instruction file at SOUL.md. With no name it connects every tool this machine has. An existing registration that already points at Synapse is left alone. Synapse never edits a tool's configuration itself \u2014 it asks the tool to.")}
    ${command("connect --refresh", "synapse connect &lt;tool&gt; --refresh", "Apply this release's descriptor to a tool that is already connected, writing its registration again. Detection cannot tell a descriptor that moved from one that did not, so a tool set up under an older release keeps that release's answer until this is run. Nothing is removed and the tool stays connected throughout. <code>synapse doctor</code> reports <em>update available</em> when it is worth running.")}
    ${command("connect --reset", "synapse connect &lt;tool&gt; --reset", "Disconnect the tool and connect it again. Costs more than <code>--refresh</code>: disconnecting takes back the skills Synapse installed for that tool, and this restores the connection rather than the library. For when what is on disk has to go before the new answer can land.")}
