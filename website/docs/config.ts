  ...
  body: `
    ...
    <table>
      ...
      <tbody>
        ...
        <tr><td><code>SYNAPSE_PAGE</code></td><td>Chooses which dashboard page opens first: <code>map</code>, <code>memory</code>, <code>mesh</code>, <code>console</code>, <code>skills</code>, <code>vaults</code>, or <code>settings</code>. Anything else opens Connections.</td><td>Opening the app straight to the screen you want.</td></tr>
        ...
        <tr><td><code>PI_CODING_AGENT_DIR</code></td><td>pi's own variable for its agent directory, which Synapse reads for the same reason: detection, the instruction pointer, and the skills folder.</td><td>A non-default pi installation.</td></tr>
        <tr><td><code>SYNAPSE_PI_PACKAGE</code></td><td>Replaces the package a connection to pi installs, and the source detection compares against. Defaults to <code>npm:synapse-pi</code>.</td><td>Working on the package from a checkout instead of npm.</td></tr>
        <tr><td><code>SYNAPSE_COMMAND</code></td><td>The Synapse executable a launched pi session should talk to.</td><td>Set automatically by <code>synapse launch pi</code> so a session reaches the binary that started it.</td></tr>
...
SYNAPSE_BIN="$HOME/bin/synapse" synapse install`)}
    ...
    <p>pi is the exception, because its connection is a package rather than a server command: detection reads pi's <code>packages</code> list and reports connected when the Synapse package is there. The extension in that package starts the same <code>synapse mcp</code> server and finds the executable itself, so there is no stored path to go stale.</p>
    <p>Each instruction file contains a managed pointer to the central <code>SOUL.md</code>. A normal sync preserves other text in place. The optional consolidation action first moves that text into <code>SOUL.md</code>, then leaves the global files pointer-only so shared guidance has one editable source.</p>
