...
export const config: Page = {
  ...
  description: "Find every Synaps-owned path, understand environment overrides, and know how integration and editor writes are protected.",
  kind: "docs",
  toc: [
  ...
  body: `
    ...
    <table>
      ...
      <tbody>
        ...
        <tr><td>Data directory</td><td><code>~/Library/Application Support/synaps</code></td></tr>
        <tr><td>Database</td><td><code>~/Library/Application Support/synaps/brain.db</code></td></tr>
        <tr><td>Installed CLI</td><td><code>~/.local/bin/synaps</code></td></tr>
      </tbody>
    </table>
    ...
    <table>
      ...
      <tbody>
        <tr><td><code>SYNAPS_DATA</code></td><td>Replaces the application-data directory.</td><td>Development, tests, or a deliberately isolated database.</td></tr>
        <tr><td><code>SYNAPS_HOME</code></td><td>Replaces the home directory used for tool files and the default CLI path.</td><td>Integration testing without touching real user files.</td></tr>
        <tr><td><code>SYNAPS_BIN</code></td><td>Replaces the full CLI installation destination.</td><td>Install outside <code>~/.local/bin</code>.</td></tr>
        <tr><td><code>SYNAPS_DOCUMENT</code></td><td>Launches the desktop app directly into a Markdown, TOML, JSON, or YAML document.</td><td>Internal editor entrypoint.</td></tr>
        <tr><td><code>SYNAPS_PROJECT_DIR</code></td><td>Provides the fallback project folder for MCP <code>vaultstatus</code>.</td><td>A client that launches the server outside the project working directory.</td></tr>
        <tr><td><code>SYNAPS_SHELL_ACTIVE</code></td><td>Identifies the shell hook currently evaluated in this shell.</td><td>Set automatically by the zsh, bash, or fish integration so status can report it.</td></tr>
      </tbody>
    </table>
    ${code("shell", `SYNAPS_DATA="$HOME/tmp/synapsdemo" synaps data check
SYNAPS_BIN="$HOME/bin/synaps" synaps install`)}
    <p>Overrides affect only the process that receives them. Keep the desktop app, CLI, and connected MCP server on the same <code>SYNAPS_DATA</code> value if they should share one store.</p>
    ...
    <p>Connection detection parses the actual TOML or JSON entry named <code>synaps</code>. It reports connected only when the stored command resolves to the expected executable and the arguments equal <code>["mcp"]</code>. A deleted development binary therefore appears stale instead of healthy.</p>
    ...
    <ol>
      ...
      <li>Writes the previous bytes to a sibling file ending in <code>.synapsbackup</code>.</li>
