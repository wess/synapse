...
pub fn modelfacing(guidance: &str, mesh: bool, learn: bool) -> String {
    ...
    if !merged.contains("`readmemory`") {
        merged.push_str("\n\nFor an abridged recall, use `readmemory` with the hit's id and the same project. Follow `next` offsets until null to read the exact stored text within the configured budget.\n");
    }
    if mesh {
